...
{
    //Todo 原值为负，映射到[0, 2^(x-1))，并求绝对值的位长度
	UINT16 comp_data;
	int i;
	
	//取data的补码
	comp_data = data < 0 ? ~data + 1 : data;
	//i为绝对位长度-1，高位起第一个非零位的索引即为data的绝对位长度
	for (i = 15; i >= 0; i--) {
		if ((comp_data & (1 << i)) != 0)
			break;
	}
 ...
 */
void
jpeg_compress(compress_io *cio, INT16 *data, INT16 *dc, BITS *dc_htable, BITS *ac_htable)
{
...
jpeg_encode(compress_io *cio, bmp_info *binfo)
{
    ...
    //Todo 调用jpeg_compress()进行压缩，
	//调用write_bits()写入压缩后的数据，
	//write_align_bits()对尾部进行填充
